...
Email notifications are sent if EMAIL_ENABLED=true in .env configuration.
"""
...
async def submit_contact(form: ContactForm):
    ...
    Email notification is handled by backend via configured SMTP service.
    Returns confirmation with submission ID.
    
    Response:
        - status: "received" if successful
        - id: MongoDB document ID
        - message: Confirmation message
        - email_sent: Boolean indicating if notification email was sent
    """
    # Save to MongoDB
    doc = form.model_dump()
    ...
    try:
        ...
        if email_sent:
            await EmailService.send_autorely(
                recipient_email=form.from_email,
                recipient_name=form.from_name,
            )
    except Exception as e:
        logger.error(f"Error sending emails: {str(e)}")
        # Don't fail the API response if email fails - form is still saved
